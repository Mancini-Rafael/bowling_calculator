...
describe ScoresParser do
  ...
  context "#parse" do
    context "when handling invalid data" do
      ...
      it "raises errors for more than 10 frames" do
        fake_scores = Array.new(11, { player: "Jeff", score: "10"} )
        parser = ScoresParser.new(scores: fake_scores)
        expect { parser.parse }.to raise_error(RuntimeError, "Too many frames in card for player Jeff")
      end
    end
    context "when handling valid data" do
      it "returns structured score information" do
        ...
        expect(scores["Jeff"].length).to eq(6)
        expect(scores["John"].length).to eq(6)
      end
    end
  end
end
